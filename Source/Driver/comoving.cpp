...
void
Nyx::checkpoint_z_est_time_step (Real& dt_0, bool& dt_changed)
{
    Real dt = dt_0;
    Real a_old, z_old, a_new, z_new;
    Real z_value;
    ...
    // Find the relevant entry of the checkpoint_z_values array
    bool found_one = false;
    for (int i = 0; i < checkpoint_z_values.size(); i++)
    {
        // We have gone from before to after one of the specified values
        if ( (z_new - checkpoint_z_values[i]) * (z_old - checkpoint_z_values[i]) < 0 && !found_one)
        {
            z_value   = checkpoint_z_values[i];
            found_one = true;
        }
    }
    ...
    if (found_one)
    {
        ...
        if (verbose && ParallelDescriptor::IOProcessor())
        {
            std::cout << " " << std::endl;
            std::cout << " ... modifying time step from " << dt_0 << " to " << dt << std::endl;
            std::cout << " ... in order to write a checkpoint file at z = " << z_value << std::endl;
            std::cout << " " << std::endl;
        }
    ...
    if (found_one)
    {
        ...
        if (verbose && ParallelDescriptor::IOProcessor())
        {
            std::cout << " " << std::endl;
            std::cout << " ... modifying time step from " << dt_0 << " to " << 0.5 * two_dt << std::endl;
            std::cout << " ... in order to write a checkpoint file at z = " << z_value 
                      << " two steps from now " << std::endl;
            std::cout << " " << std::endl;
        }
